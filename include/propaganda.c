...
char* ftos(float n, int l) {
    ...
    res_s = pstrcat(res_s, newstr("."));

    for (int i = 0; i < l-1; i++) {
        n *= 10;
        res_s = pstrcat(res_s, itos((int) n));
        n -= (int) n;
    }
...
float stof(char* str) {
    ...
    for (int i = 0; i < pstrlen(after_c); i++) {
